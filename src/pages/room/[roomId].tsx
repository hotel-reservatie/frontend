...
const Room: NextPage = () => {
  ...
  return (
    <PageLayout>
      ...
      <div className="md:grid md:grid-cols-2 md:mt-16 md:gap-x-16 items-center">
        <img
          className="rounded-3xl"
          src={data?.getRoomById?.images ? data.getRoomById.images[0] : ''}
          alt=""
        />
        <div className="grid grid-rows-3 items-center gap-9">
          <h2 className=" text-blue-400 mb">
            <span className=" text-2xl font-bold text-blue-700">{`€${data?.getRoomById?.currentPrice}`}</span>{' '}
            per night
          </h2>
          ...
          <ul className="grid grid-cols-auto auto-rows-fr gap-x-6 gap-y-4 mt-4">
            {data?.getRoomById?.facilities
              ? data.getRoomById.facilities.map(f => {
                  return (
                    <li className="flex gap-2 overflow-x-hidden">
                      {' '}
                      <div>
                        <MdDone size={24} />
                      </div>
